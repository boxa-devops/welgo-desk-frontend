/**
 * Renders the LLM ai_analysis output.
 *
 * Actual LLM output format:
 *   [Лучший выбор] HOTEL NAME: description. Красный флаг: ...
 *   [Самый дешёвый] HOTEL: description.
 *   Сравнение: ...
 *   **Рекомендация:** ...
 *
 * Also handles fallback markdown table format, headings, lists, ⚠️ warnings.
 */
...
// Split "main text. Красный флаг: flag text" into parts
...
// ── Parse raw text → typed block objects ──
function parseToBlocks(raw) {
  ...
  for (const line of lines) {
    ...
    if (compM) {
      flushList();
      blocks.push({ type: 'comparison', text: compM[1] });
      inActions = false;
      continue;
    }
    ...
    if (REC_RE.test(t)) {
      flushList();
      blocks.push({ type: 'rec', text: t.replace(REC_RE, '') });
      inActions = false;
      continue;
    }

    // **Что можно сделать**
    if (ACTIONS_RE.test(t)) {
      flushList();
      blocks.push({ type: 'actions-heading' });
      inActions = true;
      continue;
    }
...
function renderBlocks(blocks) {
  ...
  while (i < blocks.length) {
    ...
    // Collect consecutive slot blocks → 2-column grid
    if (b.type === 'slot') {
      const group = [];
    ...
    if (b.type === 'comparison') {
      out.push(<ComparisonBlock key={i} text={b.text} />);
    } else if (b.type === 'rec') {
      out.push(<RecCallout key={i} text={b.text} />);
    } else if (b.type === 'warning') {
      out.push(<p key={i} class="dap-warning">{inlineNodes(b.text)}</p>);
      ...
      out.push(<h3 key={i} class="dap-h3">{inlineNodes(b.text)}</h3>);
    } else if (b.type === 'table') {
      out.push(<TableBlock key={i} rows={b.rows} />);
    } else if (b.type === 'paragraph') {
      out.push(<p key={i} class="dap-p">{inlineNodes(b.text)}</p>);
    }
...
function extractRecPreview(blocks) {
  const rec = blocks.find(b => b.type === 'rec');
  if (!rec) return null;
  const t = rec.text.replace(/\*\*/g, '').trim();
  return t.length > 85 ? t.slice(0, 85) + '…' : t;
}
...
export default function DeskAnalysisPanel({ text, fromCache, totalFound }) {
  ...
  const recPreview = extractRecPreview(blocks);
    ...
    <div class={`dap-root${open ? ' dap-root--open' : ''}`}>
      <button class="dap-header" onClick={() => setOpen(v => !v)} aria-expanded={open}>
        <span class="dap-header-left">
          ...
          <span class="dap-header-label">Аналитика</span>
