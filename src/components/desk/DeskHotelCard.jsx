...
const SeaIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
    stroke-width="1.8" stroke-linecap="round" width="12" height="12">
    ...
    return (
      <div class="dhcard-photo-ph" aria-hidden="true">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          <rect x="2" y="8" width="20" height="14"/><path d="M6 8V6a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2"/>
        </svg>
      </div>
    );
  }
  return (
    <img class="dhcard-photo" src={src} alt={name} loading="lazy" onError={() => setFailed(true)} />
  );
}

const SelectIcon = ({ selected }) => selected ? (
  <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13">
    <path d="M20 6 9 17l-5-5" stroke="currentColor" fill="none"
      stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>
) : (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
    stroke-width="1.8" stroke-linecap="round" width="13" height="13">
...
export default function DeskHotelCard({ hotel, onHide, selected, onSelect }) {
  ...
  return (
    <article class={`dhcard dhcard--${tier}${selected ? ' dhcard--selected' : ''}`}>
      {/* Photo + badges */}
      <div class="dhcard-photo-wrap">
        <HotelPhoto src={hotel.image_url} name={hotel.hotel_name} />
        <div class={`dhcard-tier dhcard-tier--${tier}`}>{TIER_LABELS[tier]}</div>
        <button
      ...
      <div class="dhcard-body">
        ...
        {hotel.features?.length > 0 && (
          <div class="dhcard-features">
            {hotel.features.map(f => (
              <span key={f} class="dhcard-chip">{f}</span>
            ))}
          </div>
        )}
        ...
        {dates.length > 1 && (
          <div class="dhcard-dates">
            {dates.map((d, i) => (
              <button
                key={d.date}
                class={`dhcard-date-pill${i === activeDateIdx ? ' dhcard-date-pill--active' : ''}`}
                onClick={e => { e.stopPropagation(); setActiveDateIdx(i); }}
                title={`${fmtPillDate(d.date)} — $${d.price_usd_approx.toLocaleString('en-US')}`}
              >
                <span class="dhcard-date-pill-day">{fmtPillDate(d.date)}</span>
                <span class="dhcard-date-pill-price">${d.price_usd_approx.toLocaleString('en-US')}</span>
              </button>
            ))}
          </div>
        )}
        ...
        <div class="dhcard-details">
          ...
          {displayCharter && (
            <>
              <span class="dhcard-detail-sep">·</span>
              <span class="dhcard-detail-item dhcard-detail-charter">Чартер</span>
            </>
          )}
        </div>
